...
void read_overlapping_instance(xml_node<> *node) {
    // Load variables read from the .xml file
    string name = rapidxml::get_attribute(node, "name");
    unsigned Nh = stoi(rapidxml::get_attribute(node, "Nh"));
    unsigned Nw = stoi(rapidxml::get_attribute(node, "Nw"));
    ...
    unsigned screenshots = stoi(rapidxml::get_attribute(node, "screenshots", "5"));
    ...
    // Read sample image and store to Array2D
    ...
    OverlappingWFCOptions options = {periodic_input, periodic_output, height, width, symmetry, ground, Nh, Nw};
 ...
 */
unordered_map<string, Tile<Color>> read_tiles(xml_node<> *root_node, const string &current_dir, const string &subset,
                                              unsigned size) {
    ...
    for (xml_node<> *node = tiles_node->first_node("tile"); node; node = node->next_sibling("tile")) {
        ...
        if (image == nullopt) {
            ...
            for (unsigned i = 0; i < nb_of_possible_orientations(symmetry); i++) {
                const std::string l_image_path = current_dir + "/" + name + " " + to_string(i) + ".png";
                optional<Array2D<Color>> l_image = read_image(l_image_path);
                if (l_image == nullopt) {
                    throw "Error while loading " + l_image_path;
                }
                if ((l_image->width != size) || (l_image->height != size)) {
                    throw "Image " + l_image_path + " has wrond size";
                }
                images.push_back(*l_image);
            }
...
// Initialize rand for non-linux targets
    ...
    size_t elapsed_s = std::chrono::duration_cast<std::chrono::seconds>(end - start).count();
    size_t elapsed_ms = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
