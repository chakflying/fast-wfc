...
struct EntropyMemoisation {
    ...
    std::vector<size_t> nb_patterns;  // The number of patterns present
...
class Wave {
   private:
     ...
     */
    const unsigned int nb_patterns;
   ...
   public:
    ...
    Wave(unsigned height, unsigned width, const std::vector<double> &patterns_frequencies) noexcept
        : patterns_frequencies(patterns_frequencies),
          ...
          is_impossible(false),
          nb_patterns((unsigned int)patterns_frequencies.size()),
        ...
        // Every cell has all possible patterns and the initial entropy associated
        ...
        memoisation.nb_patterns = std::vector<size_t>(width * height, nb_patterns);
     ...
     */
    bool get(unsigned i, unsigned j, unsigned pattern) const noexcept { return get(i * width + j, pattern); }
     ...
     */
    void set(unsigned i, unsigned j, unsigned pattern, bool value) noexcept { set(i * width + j, pattern, value); }
    ...
    int get_min_entropy(std::minstd_rand &gen) const noexcept {
        ...
        for (unsigned i = 0; i < size; i++) {
            ...
            // to 0).
            size_t l_nb_patterns = memoisation.nb_patterns[i];
            if (l_nb_patterns == 1) {
                continue;
            }
