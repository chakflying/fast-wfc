...
struct OverlappingWFCOptions {
    ...
    unsigned pattern_height;  // The height in pixel of the patterns.
    unsigned pattern_width;  // The width in pixel of the patterns.

    /**
     * Get the wave height given these options.
     */
    unsigned get_wave_height() const noexcept { return periodic_output ? out_height : out_height - pattern_height + 1; }

    /**
     * Get the wave width given these options.
     */
    unsigned get_wave_width() const noexcept { return periodic_output ? out_width : out_width - pattern_width + 1; }
};
...
class OverlappingWFC {
   private:
     ...
     */
    static unsigned get_ground_pattern_id(const Array2D<T> &input, const std::vector<Array2D<T>> &patterns,
                                          const OverlappingWFCOptions &options) noexcept {
        // Get the pattern.
        Array2D<T> ground_pattern =
            input.get_sub_array(input.height - 1, input.width / 2, options.pattern_height, options.pattern_width);
    ...
    static std::pair<std::vector<Array2D<T>>, std::vector<double>> get_patterns(
        ...
        std::vector<Array2D<T>> symmetries(8, Array2D<T>(options.pattern_height, options.pattern_width));
        unsigned max_i = options.periodic_input ? input.height : input.height - options.pattern_height + 1;
        unsigned max_j = options.periodic_input ? input.width : input.width - options.pattern_width + 1;

        for (unsigned i = 0; i < max_i; i++) {
            for (unsigned j = 0; j < max_j; j++) {
                // Compute the symmetries of every pattern in the image.
                symmetries[0].data = input.get_sub_array(i, j, options.pattern_width, options.pattern_height).data;
    ...
    Array2D<T> to_image(const Array2D<unsigned> &output_patterns) const noexcept {
        ...
        if (options.periodic_output) {
            ...
            for (unsigned y = 0; y < options.get_wave_height(); y++) {
                ...
                for (unsigned dx = 1; dx < options.pattern_width; dx++) {
            ...
            for (unsigned x = 0; x < options.get_wave_width(); x++) {
                ...
                for (unsigned dy = 1; dy < options.pattern_height; dy++) {
                    output.get(options.get_wave_height() - 1 + dy, x) = pattern.get(dy, 0);
                }
            }
            const Array2D<T> &pattern =
                patterns[output_patterns.get(options.get_wave_height() - 1, options.get_wave_width() - 1)];
            for (unsigned dy = 1; dy < options.pattern_height; dy++) {
                for (unsigned dx = 1; dx < options.pattern_width; dx++) {
                    output.get(options.get_wave_height() - 1 + dy, options.get_wave_width() - 1 + dx) =
                        pattern.get(dy, dx);
                }
            }
        }
